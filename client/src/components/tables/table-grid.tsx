...
export function TableGrid({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        const response = await apiRequest("GET", `https://order-mobile-be.onrender.com/api/order-items/${orderId}`);
    ...
    const handleOrderUpdate = (event: CustomEvent) => {
      ...
      if (!event.detail?.skipAllRefetch && event.detail?.orderId) {
        queryClient.invalidateQueries({
          queryKey: ["https://order-mobile-be.onrender.com/api/order-items", event.detail.orderId],
        });
      }
    };
            ...
            if (
              ...
              try {
                ...
                const [freshTables, freshOrders] = await Promise.all([
                  fetch(
                    `https://order-mobile-be.onrender.com/api/tables?_ws_refresh=${cacheBuster}&_force=true&_timestamp=${timestamp}`,
                    {
                      cache: "no-store",
                      headers: {
                  ...
                  fetch(
                    `https://order-mobile-be.onrender.com/api/orders?_ws_refresh=${cacheBuster}&_force=true&_timestamp=${timestamp}`,
                    {
                      cache: "no-store",
                      headers: {
                        "Cache-Control": "no-cache, no-store, must-revalidate",
                ...
                queryClient.setQueryData(["https://order-mobile-be.onrender.com/api/tables"], freshTables);
                queryClient.setQueryData(["https://order-mobile-be.onrender.com/api/orders"], freshOrders);
  ...
  const updateTableStatusMutation = useMutation({
    mutationFn: ({ tableId, status }: { tableId: number; status: string }) =>
      apiRequest("PUT", `https://order-mobile-be.onrender.com/api/tables/${tableId}/status`, { status }),
  ...
  const completePaymentMutation = useMutation({
    mutationFn: ({
      ...
      apiRequest("PUT", `https://order-mobile-be.onrender.com/api/orders/${orderId}/status`, {
        status: "paid",
        paymentMethod,
      }),
    onSuccess: async (data, variables) => {
        ...
        try {
          ...
          if (otherActiveOrders.length === 0) {
            ...
            try {
              await apiRequest(
                "PUT",
                `https://order-mobile-be.onrender.com/api/tables/${completedOrder.tableId}/status`,
                {
                  status: "available",
                },
              );
      ...
      try {
        ...
        const [freshTables, freshOrders] = await Promise.all([
          fetch("https://order-mobile-be.onrender.com/api/tables", {
            cache: "no-store",
            headers: { "Cache-Control": "no-cache" },
          }).then((r) => r.json()),
          fetch("https://order-mobile-be.onrender.com/api/orders", {
            cache: "no-store",
            headers: { "Cache-Control": "no-cache" },
          }).then((r) => r.json()),
        ]);
      ...
      try {
        const [completedOrder, orderItemsData] = await Promise.all([
          queryClient.fetchQuery({
            queryKey: ["https://order-mobile-be.onrender.com/api/orders", variables.orderId],
            queryFn: async () => {
              const response = await apiRequest(
                "GET",
                `https://order-mobile-be.onrender.com/api/orders/${variables.orderId}`,
              );
              return response.json();
            },
          }),
          queryClient.fetchQuery({
            queryKey: ["https://order-mobile-be.onrender.com/api/order-items", variables.orderId],
            queryFn: async () => {
              const response = await apiRequest(
                "GET",
                `https://order-mobile-be.onrender.com/api/order-items/${variables.orderId}`,
              );
              return response.json();
            },
          }),
    ...
    mutationFn: async (orderId: number) => {
      ...
      if (order?.tableId) {
        ...
        if (!otherActiveOrders || otherActiveOrders.length === 0) {
          await apiRequest("PUT", `https://order-mobile-be.onrender.com/api/tables/${order.tableId}/status`, {
            status: "available",
          });
        }
      }
  ...
  const recalculateOrderTotalMutation = useMutation({
    mutationFn: async (orderId: number) => {
      ...
      if (!remainingItems || remainingItems.length === 0) {
        ...
        const updateResult = await apiRequest("PUT", `https://order-mobile-be.onrender.com/api/orders/${orderId}`, {
          subtotal: "0",
          tax: "0",
          total: "0",
        });
    ...
    onSuccess: (data, orderId) => {
      ...
      Promise.all([
        fetch("https://order-mobile-be.onrender.com/api/orders", { cache: "no-store" }).then((r) => r.json()),
        fetch("https://order-mobile-be.onrender.com/api/tables", { cache: "no-store" }).then((r) => r.json()),
        fetch(`https://order-mobile-be.onrender.com/api/order-items/${orderId}`, { cache: "no-store" }).then((r) =>
          r.json(),
        ),
      ])
        .then(() => {
          ...
          queryClient.setQueryData(["https://order-mobile-be.onrender.com/api/orders"], (oldData: any) => {
  ...
  const handlePaymentMethodSelect = async (
    ...
    if (method === "paymentCompleted" && paymentData?.success) {
      ...
      try {
        ...
        const [freshTables, freshOrders] = await Promise.all([
          fetch(
            "https://order-mobile-be.onrender.com/api/tables?" +
              new URLSearchParams({
          ...
          fetch(
            "https://order-mobile-be.onrender.com/api/orders?" +
              new URLSearchParams({
                _t: Date.now().toString(),
                _force: "true",
              }),
  ...
  const handlePrintOrder = async (order: any) => {
    ...
    try {
      const orderItems = await queryClient.fetchQuery({
        queryKey: [`https://order-mobile-be.onrender.com/api/order-items/${order.id}`],
        queryFn: async () => {
          const response = await apiRequest(
            "GET",
            `https://order-mobile-be.onrender.com/api/order-items/${order.id}`,
      ...
      const response = await fetch("https://order-mobile-be.onrender.com/api/auto-print", {
        method: "POST",
        headers: {
        ...
        <ReceiptModal
          ...
          onClose={async () => {
            ...
            try {
              ...
              const timestamp = Date.now().toString();
              const [freshTables, freshOrders] = await Promise.all([
                fetch(`https://order-mobile-be.onrender.com/api/tables?_t=${timestamp}&_force=refresh`, {
                  cache: "no-store",
                  headers: {
                    "Cache-Control": "no-cache, no-store, must-revalidate",
                    Pragma: "no-cache",
                ...
                fetch(`https://order-mobile-be.onrender.com/api/orders?_t=${timestamp}&_force=refresh`, {
                  cache: "no-store",
                  headers: {
                    "Cache-Control": "no-cache, no-store, must-revalidate",
                    Pragma: "no-cache",
